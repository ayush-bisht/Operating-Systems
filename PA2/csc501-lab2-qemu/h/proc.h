...
#define PRLOCKWAIT	'\011' 		/* process waiting for a lock	*/
...
struct	pentry	{
	...
	int 	plock[NLOCK];	//all the locks the process has acquired
	int	plockret[NLOCK];
	int	pinh; 		//inherited prio	
	int	lockid;		//desc of the lock, the process is waiting on
};
