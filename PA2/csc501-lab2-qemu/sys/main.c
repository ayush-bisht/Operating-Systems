...
void writer4a (char *msg, int lck)
{
        kprintf ("  %s: to acquire lock %d\n", msg, lck);
        lock (lck, WRITE, DEFAULT_LOCK_PRIO);
	kprintf ("  %s: acquired lock %d\n", msg, lck);
...
void test4()
{
	...
	assert (lck2 != SYSERR, "Test 4 failed");
       ...
       // rd2 = create(reader3, 2000, 30, "reader3", 2, "reader B", lck);
        wr1 = create(writer4a, 2000, 40, "writer4", 2, "writer A", lck);
...
int reader5a(char* msg, int lck, int lck1){
        int     ret, ret1, ret2;
...
void test5(){
	...
	rd1 = resume(create(reader5, 2000, 30, "reader5", 2, "reader A", lck));
        rd2 = resume(create(reader5a, 2000, 30, "reader5", 3, "reader B", lck, lck1));
	resume(rd1);
	sleep(1);
	wr2 = resume(create(writer5a, 2000, 30, "writer5", 2, "writerA", lck)); 
	sleep(1);
	resume(rd2);
	sleep(1);
	lck = lcreate();
	kprintf("lock created: %d \n", lck);
	wr1 = resume(create(writer5, 2000, 30, "writer5", 2, "writerB", lck));
        sleep(20);
}
#endif
int main()
