...
//assign a page directory with top 4 entries as the global page tables
        pd_t *pd_entry;
        fr_map_t *frmptr;
        int avail;
        get_frm(&avail);
//        kprintf("PD of proc %d, created at frame %d, address %0x \n", pid, avail, (avail+FRAME0)*NBPG);
        frmptr = &frm_tab[avail];
        frmptr->fr_status = FRM_MAPPED;
        frmptr->fr_pid = pid;
        frmptr->fr_type = FR_DIR;
        frmptr->fr_dirty = 0;
	frmptr->fr_vpno = -1;
	proctab[pid].pdbr = (avail+FRAME0)*NBPG;
        pd_entry = (avail+FRAME0)*NBPG;
        for(i=0; i< NENTRY; i++)
        {
                pd_entry->pd_write = 1;
                pd_entry->pd_user = 0;
                ...
                {
                        pd_entry->pd_pres = 1;
                        if(i==0)        pd_entry->pd_base = FRAME0 + 1;
                        else if(i==1)   pd_entry->pd_base = FRAME0 + 2;
                        else if(i==2)   pd_entry->pd_base = FRAME0 + 3;
                        else if(i==3)   pd_entry->pd_base = FRAME0 + 4;
                }
                pd_entry++;
        }
