...
//initilaize bsm and frame tab 
bs_map_t bsm_tab[BS_MAX];
...
//initialize bs tab and frame tab	
	init_bsm();
	init_frm();
	
//initialize the circular queue for SC policy
	front = rear = NULL;	
	debug_mode = 0;

//setting up the page dir for null process
	...
	int avail;
        ...
        proctab[0].pdbr = (avail+FRAME0)*NBPG;
		...
		pd_entry->pd_pres = 1;
                ...
                pd_entry->pd_avail = 0;
		pd_entry->pd_base = FRAME0 + i + 1;
                pd_entry++;
        }

//setting the four global page tables
